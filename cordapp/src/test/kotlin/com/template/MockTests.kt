...
class ResolveTransactionsFlowTest {
    ...
    fun setup() {
        ...
        bod.internals.registerInitiatedFlow(AlokIssueRequest::class.java)
        bod.internals.registerInitiatedFlow(AlokMoveResponse::class.java)
        corp.internals.registerInitiatedFlow(AlokMoveResponse::class.java)
    }
    ...
    fun `simple issuance flow`() {
        val p = AlokIssueRequest("TEST THOUGHT", bod.info.chooseIdentity())
        val future = corp.services.startFlow(p).resultFuture
        mockNet.runNetwork()
        val results = future.getOrThrow()
        val ds = results.tx.outputStates[0] as AlokState
        assertEquals("TEST THOUGHT", ds.thought)
    }

    @Test
    fun `invalid issuer`() {
        val p = AlokIssueRequest("TEST THOUGHT", invalidIssuer.info.chooseIdentity())
        try {
            val future = corp.services.startFlow(p).resultFuture
            mockNet.runNetwork()
            future.getOrThrow()
    ...
    fun `simple issue-and-move flow`() {
        val p = AlokIssueRequest("IF WORK IS ENERGY EXPENDED OVER TIME WHY DON'T WE GET PAID IN CALORIES", bod.info.chooseIdentity())
        val future = corp.services.startFlow(p).resultFuture
        mockNet.runNetwork()
        val results = future.getOrThrow()
        val ds = results.tx.outputStates[0] as AlokState
    ...
    fun `correct DanielState recorded in vault`() {
        ...
        for(node in listOf(corp, bod)) {
            node.database.transaction {
                val states = node.services.vaultService.queryBy<AlokState>().states
